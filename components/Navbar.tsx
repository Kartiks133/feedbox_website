	...
	useEffect(() => {
		const handleClickOutside = (event) => {
			if (
				...
				!eventsMenuRef.current.contains(event.target)
			) {
				setEventsOpen(false);
			}
		};
			...
			<div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between text-white">
				...
				<div className="flex items-center gap-10">
					...
					<div className="hidden md:flex gap-6 items-center text-sm font-medium tracking-wide">
						...
						<Link href="/community" className="hover:text-cyan-400 transition">
							Community
						</Link>
							...
							<AnimatePresence>
								{eventsOpen && (
									...
									>
										<Link
											href="#"
											className="block px-4 py-2 hover:bg-cyan-500/20"
										>
											Upcoming
										</Link>
										<Link
						...
						<Link href="/blogs" className="hover:text-cyan-400 transition">
							Blogs
						</Link>
					</div>
				</div>
				...
				<div className="flex items-center gap-4">
					<Link href="/login">
						<button className="border border-cyan-400 px-4 py-2 rounded hover:bg-cyan-500/20 text-sm">
							Sign In
						</button>
					</Link>
					<Image
